...
class CX
{
	...
	public function inject( &$property, $componentKey, $params = [] )
	{
		$params = $this->_loadParams( $componentKey, $params );

		if( isset( $this->_passive[$componentKey] ) )
		{
			$class = $this->_passive[$componentKey];
		...
		$config = $this->_loadConfig( $componentKey.'/_loader' );

		//component must be created and assigned before intializing
		$property = $this->_create( $componentKey, $config, $params );
	...
	protected function _create( $componentKey, $config, $params )
	{
		$class = $config['class'];
		...
		if( isset( $config['create'] ) )
		{
			...
			$component = $creator( $component, $params );
		}
